...
to add `PlanGraph` as a first-class local domain contract generated by an
inspectable planner node from the TaskBrief before planned execution nodes are
scheduled as primary TaskRuns. `ExecutionTraceGraph` remains a read model
projected from existing durable evidence and dynamic trace nodes. Trace
