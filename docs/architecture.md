...
## PlanGraph And Execution Trace Boundary

`docs/plan-graph-execution-trace-product.md` defines the split between
pre-execution planning and actual execution evidence. `PlanGraph` is now a
first-class local domain contract generated by an inspectable planner node from
the TaskBrief before primary TaskRuns are bound to planned executable nodes.
`ExecutionTraceGraph` remains a read model projected from existing durable
evidence and dynamic trace nodes. Trace
projection should read task runs, run events, artifacts, RoleCalls,
RoleCallEvents, RoleTodos, verification rows, risk reports, review artifacts,
run metadata, RoleCall tool events, and plan/trace links instead of asking an
LLM to reconstruct what happened.

Legacy tasks with runs or RoleCalls but no PlanGraph stay readable through the
compatibility projection over existing RoleCall/run evidence, and renderers
avoid presenting RoleCalls as the complete graph model.
...
starts. `plannerAgentKind` can choose the planner adapter; when omitted it
follows the primary adapter. Planner persona, role metadata, system prompt,
and output contract are generated by Agent Hub and are not user-customizable.
`disabled` keeps the pre-PlanGraph path available for dry runs, legacy callers,
and internal fake-adapter tests. Unsupported planner modes fail inspectably
instead of activating caller-supplied graphs. Agent-backed planner output must
be structured JSON matching the current task/version/id and is rejected before
any primary adapter execution when it is invalid. Planner events are marked as
planner-phase events and filtered out of assistant-facing output so PlanGraph
JSON does not become a role response. The planner is represented as the
graph's `planner` node with `execution.mode: "system"`. TaskRunner selects the
first planned `primary_run` node as the current primary run binding, persists
that binding in `run_metadata.plan_binding_json`, emits planner lifecycle
events, and passes the active PlanGraph, current plan node, and allowed next
node ids through adapter runtime input. Additional primary PlanNodes can still
be scheduled by calling TaskRunner with `RunTaskInput.planGraphBinding`; the
higher-level
...
renderer and read model keep default graph text title-first: step-grouped flow
rows, outgoing edge labels, toolbar focus, and relationship rows use
plan/trace titles. Wide rows render visible ranks as top-down `step` groups,
show same-rank lanes as sibling node rows, and keep local link rows indented
...
- `@planner` is a special system product role for plan creation. It is always
  executed by an agent/LLM adapter for real runs; users may choose the adapter
  but not the planner persona, prompt, role metadata, or output contract.
...
lines, evidence lines, and inline diff references. Work block message lines keep
their full text and trailing-whitespace-only normalization in core; display
width wrapping and any visible viewport clipping belong to the Ink renderer.
Ink renders selected Work blocks through a dedicated block-list renderer with a
title row, time gutter, speaker column, status token column, selected-block
frame, folded metadata affordances, dense pending-review collapse, and bounded
