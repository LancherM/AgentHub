...
`packages/core/src/plan-graph-planner.ts` provides deterministic planning plus
structured planner-output parsing. TaskRunner builds the existing runtime
context and TaskBrief, then creates, validates, and persists an active
PlanGraph before selecting and starting the primary adapter. The default
`RunTaskInput.planGraphMode` is deterministic, `disabled` keeps the
pre-PlanGraph execution path available for compatibility tests and legacy
callers, `manual` accepts a caller-supplied PlanGraph after validation, and
`agent_adapter` runs the special local `@planner` role through a configured
adapter in its own isolated planner worktree before the primary run starts.
Agent-backed planner output must be structured JSON matching the current
task/version/id and is rejected before any primary adapter execution when it is
invalid. The planner is represented as the graph's `planner` node with
`execution.mode: "system"`. TaskRunner selects the first planned `primary_run`
...
- `@planner` is a special product role for plan creation. Deterministic,
  manual, and opt-in local adapter-backed planner modes all feed the same
  validation and persistence path while recording planner ownership in the
  persisted PlanGraph and planner lifecycle run events.
