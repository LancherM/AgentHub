...
assistant results when the callee completes. A one-click Role Details inspector
exposes the RoleCall graph, todo ledger, event stream, linked evidence,
placeholder retry/cancel/approval actions, and bounded raw JSON without
requiring the renderer to read SQLite, filesystem, shell, Git, or child-process
state directly.
...
evidence. The role must not simulate hidden subagents or discover role syntax
by reading the repository; Agent Hub owns delegation and audit state.
