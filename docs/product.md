...
planner node plus one required executable role node. Requests that are only
testing or demonstrating PlanGraph, Graph, role delegation, or simple planning
behavior are treated as no-repository-change unless the user explicitly asks
for file edits. Repository workflow rules about commits, pushes, pull requests,
