...
planner node plus one required executable role node, while repository-changing
tasks may add verification or review nodes only when configured commands,
changed-file risk, or the user's request justify them. The prompt also includes
the current verification-command context, tells the planner to model runnable
verification as `primary_run`, and prevents required `manual` gates from being
