...
describe("CLI", () => {
  ...
  it("keeps ad-hoc SQLite projects scoped to their repository roots", async () => {
    ...
    await expect(
      main(["run", "--repo", firstProjectRoot, "@fake", "first ad-hoc run"], io, firstProjectRoot, runtime)
    ).resolves.toBe(0);
    await expect(
      main(["run", "--repo", secondProjectRoot, "@fake", "second ad-hoc run"], io, secondProjectRoot, runtime)
    ).resolves.toBe(0);
