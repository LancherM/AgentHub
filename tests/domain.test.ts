...
describe("domain model validation", () => {
  ...
  it("rejects unsafe PlanGraph instructions and unsupported execution modes", () => {
    ...
    const allowedMemoryProposal = planGraphFixture();
    allowedMemoryProposal.nodes[2] = {
      ...allowedMemoryProposal.nodes[2],
      kind: "memory",
      role: "memory",
      title: "Record memory proposal candidate",
      instructions:
        "Summarize a memory proposal candidate for explicit review without changing memory state.",
      acceptanceCriteria: ["Memory proposal candidates are recorded as proposals only."],
      riskLevel: "low",
      required: false,
      execution: { mode: "non_executable" }
    };
    expect(validatePlanGraph(allowedMemoryProposal)).toBe(allowedMemoryProposal);

    const unsafeMemoryApproval = planGraphFixture();
    unsafeMemoryApproval.nodes[2] = {
      ...unsafeMemoryApproval.nodes[2],
      kind: "memory",
      role: "memory",
      title: "Approve memory",
      instructions: "Approve memory generated by the run."
    };
    expect(() => validatePlanGraph(unsafeMemoryApproval)).toThrow(
      DomainValidationError
    );
